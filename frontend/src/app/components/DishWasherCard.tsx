...
export default function DishWasherCard({
  onResultHook,
}: {
  ...
  const [state, setState] = useState<DishWasherState>({
    isOn: false,
    isLoaded: false,
  });

  const onSubmit = () => {
    fetch(`${process.env.NEXT_PUBLIC_SERVER_URL ?? ""}/state/dish-washer`, {
      method: "PUT",
      headers: {
  ...
  return (
    <HomeStateCardTemplate title="Dish Washer" onSubmit={onSubmit}>
      <Stack
        direction="row"
